...
-- Adicione esta nova categoria aos Themes (após a categoria Halloween)
Purple = {
    ["Color Hub 1"] = ColorSequence.new({
    ...
    ["Color Hub 2"] = Color3.fromRGB(30, 30, 30),
    ["Color Stroke"] = Color3.fromRGB(40, 40, 40),
    ["Color Theme"] = Color3.fromRGB(255, 115, 0),-- 179, 0, 255
...
Halloween = {
    ["Color Hub 1"] = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(40, 15, 50)),
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(20, 10, 10))
    }),
    ["Color Hub 2"] = Color3.fromRGB(25, 15, 30),
    ["Color Stroke"] = Color3.fromRGB(60, 20, 80),
    ["Color Theme"] = Color3.fromRGB(255, 140, 0),
    ["Color Text"] = Color3.fromRGB(250, 240, 200),
    ["Color Dark Text"] = Color3.fromRGB(200, 120, 100)
}
	},
	Info = {
		Version = "1.2.0"
...
local PumpkinConfig = {
	MaxPumpkins = 10,
	SpawnRate = 1.2,
	PumpkinSize = {Min = 40, Max = 70},
	PumpkinSpeed = {Min = 10, Max = 20},
	PumpkinLifetime = 10,
	PumpkinIDs = {
		"rbxassetid://11559843780621",
		"rbxassetid://11559843780621",
		"rbxassetid://3518927409",
		"rbxassetid://3518927409"
	},
	FloatAmplitude = 2,
	FloatFrequency = 2,
...
local function UpdateCyberpunkParticles(deltaTime)
	if not ParticleContainer or not ParticleContainer.Parent then
		for i = #ActiveParticles, 1, -1 do
			if ActiveParticles[i].Frame then
				ActiveParticles[i].Frame:Destroy()
			end
	...
	for i = #ActiveParticles, 1, -1 do
		...
		if particle.Frame and particle.Frame.Parent then
			...
			local newY = currentPos.Y.Offset + (particle.Direction.Y * deltaTime)
			
			if elapsed >= ParticleConfig.ParticleLifetime or newY < -20 then
...
local function SpawnCyberpunkSystem()
	...
	local currentTime = tick()
	if currentTime - LastSpawn >= ParticleConfig.SpawnRate then
		CreateCyberpunkParticle()
		LastSpawn = currentTime
	end
end

local function CreatePumpkinParticle()
	if #ActivePumpkins >= PumpkinConfig.MaxPumpkins then return end
	if not ParticleContainer or not ParticleContainer.Parent then return end
	...
	local startX = math.random(10, math.max(11, containerSize.X - 10))
	local startY = containerSize.Y + 60
	local size = math.random(PumpkinConfig.PumpkinSize.Min, PumpkinConfig.PumpkinSize.Max)
	local speed = math.random(PumpkinConfig.PumpkinSpeed.Min, PumpkinConfig.PumpkinSpeed.Max)
	local imageID = PumpkinConfig.PumpkinIDs[math.random(1, #PumpkinConfig.PumpkinIDs)]
	local rotationSpeed = PumpkinConfig.RotationEnabled and math.random(PumpkinConfig.RotationSpeed.Min, PumpkinConfig.RotationSpeed.Max) or 0
	
	local Pumpkin = Create("ImageLabel", ParticleContainer, {
		Size = UDim2.fromOffset(size, size),
		Position = UDim2.fromOffset(startX, startY),
		BackgroundTransparency = 1,
		Image = imageID,
	...
	local PumpkinData = {
		...
		FloatOffset = math.random(0, 360),
		InitialX = startX
	}
	
	CreateTween({Pumpkin, "ImageTransparency", 0.1, 0.8})
	
	table.insert(ActivePumpkins, PumpkinData)
end

local function UpdatePumpkinParticles(deltaTime)
	if not ParticleContainer or not ParticleContainer.Parent then
		for i = #ActivePumpkins, 1, -1 do
			if ActivePumpkins[i].Frame then
				ActivePumpkins[i].Frame:Destroy()
			end
			table.remove(ActivePumpkins, i)
		end
		return
	end
	...
	for i = #ActivePumpkins, 1, -1 do
		local pumpkin = ActivePumpkins[i]
		
		if pumpkin.Frame and pumpkin.Frame.Parent then
			local elapsed = currentTime - pumpkin.StartTime
			local currentPos = pumpkin.Frame.Position
			local newY = currentPos.Y.Offset + (pumpkin.Direction.Y * deltaTime)
			
			if elapsed >= PumpkinConfig.PumpkinLifetime or newY < -70 then
				CreateTween({pumpkin.Frame, "ImageTransparency", 1, 0.6, true})
				task.delay(0.6, function()
					if pumpkin.Frame and pumpkin.Frame.Parent then
						pumpkin.Frame:Destroy()
					end
				end)
				table.remove(ActivePumpkins, i)
			else
				local floatX = math.sin((currentTime * PumpkinConfig.FloatFrequency) + pumpkin.FloatOffset) * PumpkinConfig.FloatAmplitude
				local newX = pumpkin.InitialX + floatX
				
				pumpkin.Frame.Position = UDim2.fromOffset(newX, newY)
				
				if PumpkinConfig.RotationEnabled then
					pumpkin.Frame.Rotation = (pumpkin.Frame.Rotation + (pumpkin.RotationSpeed * deltaTime * 60)) % 360
				end
			end
		else
			table.remove(ActivePumpkins, i)
...
local function SpawnPumpkinSystem()
	if not MainFrame or not MainFrame.Visible then return end
	
	local currentTime = tick()
	if currentTime - LastPumpkinSpawn >= PumpkinConfig.SpawnRate then
		CreatePumpkinParticle()
		LastPumpkinSpawn = currentTime
	end
end

local ParticleConnection = RunService.Heartbeat:Connect(function(deltaTime)
	UpdateCyberpunkParticles(deltaTime)
	SpawnCyberpunkSystem()
	UpdatePumpkinParticles(deltaTime)
	SpawnPumpkinSystem()
end)
...
function Tab:AddLabel(Configs)
    local LabelText = (type(Configs) == "string" and Configs)
        or (Configs and (Configs[1] or Configs.Name or Configs.Title))
        or "Label"

    -- Fundo do label (igual aos outros elementos)
    local Frame = InsertTheme(Create("Frame", Container, {
        Size = UDim2.new(1, 0, 0, 25),
        BackgroundColor3 = Theme["Color Hub 2"],
        BorderColor3 = Theme["Color Stroke"],
        Name = "Option",
    }), "Background")

    -- Canto arredondado
    Create("UICorner", Frame, { CornerRadius = UDim.new(0, 4) })
    ...
    -- Guarda referência como nas outras opções
    ...
    function Label:Visible(bool)
        if bool == nil then
            ...
            Funcs:ToggleVisible(Frame, bool)
        end
    end

    function Label:SetColor(color3)
